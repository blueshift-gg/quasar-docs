...
The `set_inner` account opt-in generates `EscrowInner` with the native field types. Passing that value writes every field to the account buffer. `MakeBumps` is auto-generated and contains bump seeds for every PDA in the struct.
