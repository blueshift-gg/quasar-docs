...
- Declare and validate a typed PDA seed layout
...
- **`user: Signer`** -- The wallet depositing SOL. `Signer` verifies the transaction signature. `#[account(mut)]` requires a writable account because its lamports decrease during transfer.
- **`vault: UncheckedAccount`** -- The vault PDA receiving SOL. `address = VaultPda::seeds(user.address())` verifies the typed PDA derivation and records the bump in `ctx.bumps.vault`. We use `UncheckedAccount` because the vault holds only lamports -- no data to deserialize.
- **`system_program: Program<SystemProgram>`** -- Required for the transfer CPI. `Program<SystemProgram>` validates this is the system program.
...
`#[seeds(b"vault", user: Address)]` declares the seed layout once.
`VaultPda::seeds(user.address())` supplies the typed value for each instruction,
verifies the passed account, and records the bump in `ctx.bumps`. See [Program
Derived Addresses](/docs/core-concepts/pda).
