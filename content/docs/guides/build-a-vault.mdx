...
- **`vault: UncheckedAccount`** -- The vault PDA receiving SOL. `seeds = [b"vault", user]` verifies the PDA derivation. `bump` derives and stores the bump seed in `ctx.bumps`. We use `UncheckedAccount` because the vault holds only lamports -- no data to deserialize.
