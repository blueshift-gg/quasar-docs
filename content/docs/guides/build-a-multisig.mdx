...
records the verified bump in `ctx.bumps.config`. Note
`Account<MultisigConfig<\'info>>` (not a reference) -- required for dynamic
fields because the framework needs ownership to manage reallocation.
