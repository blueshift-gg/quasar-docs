...
Providing a name fills the project-name prompt, but the CLI still asks for the remaining choices. Add `--yes` to use explicit flags and saved defaults without prompting:
...
Lock the new instruction's auto discriminator before you build or deploy:
...
The packaged release rehearsal runs the non-interactive minimal init, add, discriminator lock, release build, and three test commands shown above. Deployment is intentionally excluded because it requires a funded keypair and an external cluster.
...
- **[Core Concepts](/docs/core-concepts/program-structure)** -- Zero-copy accounts, discriminators, CPI, and PDAs.
- **[Account Constraints](/docs/references/account-constraints)** -- typed seeds, `has_one`, `constraints`, `init`, `close`, and more.
