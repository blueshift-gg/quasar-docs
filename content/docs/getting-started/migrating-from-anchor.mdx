---
title: Migrating from Anchor
description: What changes when you port an Anchor program to Quasar.
---

![Equivalent Anchor and Quasar program patterns shown side by side](/images/migrating-from-anchor.webp)

Quasar shares Anchor's high-level accounts and instruction model, but v0.1 has
its own account directive grammar. It is also zero-copy and `no_std`, so there
is no deserialization and no heap allocation.

## Imports

```rust
// anchor_lang::prelude::*;
use quasar_lang::prelude::*;
```

## Instructions

```rust
#[instruction(discriminator = 0)]
pub fn make(ctx: Ctx<Make>, deposit: u64, receive: u64) -> Result<(), ProgramError> {
    ctx.accounts.make_escrow(receive, &ctx.bumps)?;
    ctx.accounts.deposit_tokens(deposit)
}
```

| Anchor | Quasar |
|--------|--------|
| `Context<T>` | `Ctx<T>` |
| `Result<()>` | `Result<(), ProgramError>` |
| Discriminator optional (defaults to SHA-256 hash) | Discriminator **required** -- no implicit hashing |

Logic lives in `impl` methods on the accounts struct. The `#[program]` block is just dispatch.

## Accounts struct

```rust
#[derive(Accounts)]
pub struct Take {
    #[account(mut)]
    pub taker: Signer,
    #[account(
        mut,
        has_one(maker),
        constraints(escrow.receive > 0),
        close(dest = taker),
        address = Escrow::seeds(maker.address())
    )]
    pub escrow: Account<Escrow>,
    #[account(mut)]
    pub maker: UncheckedAccount,
    pub token_program: Program<TokenProgram>,
    pub system_program: Program<SystemProgram>,
}
```

| Anchor | Quasar |
|--------|--------|
| `Signer<'info>` | `Signer` |
| `Account<'info, T>` | `Account<T>` |
| `#[account(mut)]` attribute | `#[account(mut)]` attribute |
| `seeds = [b"escrow", maker.key().as_ref()], bump` | `#[seeds(b"escrow", maker: Address)]` on `Escrow`, then `address = Escrow::seeds(maker.address())` on the field |

## Account types

```rust
#[account(discriminator = 1)]
#[seeds(b"escrow", maker: Address)]
pub struct Escrow {
    pub maker: Address,
    pub mint_a: Address,
    pub receive: u64,
    pub bump: u8,
}
```

| Anchor | Quasar |
|--------|--------|
| `#[account]` | `#[account(discriminator = N)]` -- explicit, must be non-zero |
| Borsh deserialization | Zero-copy -- fields read directly from account memory |

`Address` replaces `Pubkey` and `.address()` replaces `.key()` -- this is a Solana SDK change, not Quasar-specific.

## CPI

Method-style calls instead of `CpiContext`:

```rust
// Transfer tokens (no PDA signer)
self.token_program
    .transfer(&self.maker_ta_a, &self.vault_ta_a, &self.maker, amount)
    .invoke()
```

```rust
// PDA-signed transfer + close
let escrow_signer = self.escrow_signer(bumps);

self.token_program
    .transfer(&self.vault_ta_a, &self.taker_ta_a, &self.escrow, amount)
    .invoke_signed(&escrow_signer)?;

self.token_program
    .close_account(&self.vault_ta_a, &self.taker, &self.escrow)
    .invoke_signed(&escrow_signer)
```

`#[derive(Accounts)]` generates `self.escrow_signer(bumps)` from the field's
typed address constraint. It rebuilds the declared seeds and appends the
verified bump for `invoke_signed`.

## Events

```rust
#[event(discriminator = 0)]
pub struct MakeEvent {
    pub escrow: Address,
    pub maker: Address,
    pub deposit: u64,
}

// Emit via sol_log_data (~100 CU)
emit!(MakeEvent {
    escrow: *self.escrow.address(),
    maker: *self.maker.address(),
    deposit: amount,
});
```

Same as Anchor but with an explicit discriminator. Supported field types: `Address`, `u8`--`u128`, `i8`--`i128`, `bool`.

## Error codes

```rust
#[error_code]
pub enum MyError {
    Unauthorized,
    InsufficientFunds,
}

require!(amount > 0, MyError::InsufficientFunds);
```

`require!`, `require_eq!`, and `require_keys_eq!` work the same. Variants map to `ProgramError::Custom(N)` with an offset of 6000.

## `no_std`

Quasar programs are `#![no_std]` on-chain (`std` is available during tests via `cfg_attr`).

No `std::string::String`, `std::vec::Vec`, or heap allocation. Quasar provides its own zero-copy replacements for use in account structs:

```rust
#[account(discriminator = 1)]
pub struct MultisigConfig {
    pub creator: Address,
    pub threshold: u8,
    pub label: String<32>,         // max 32 bytes, read through label()
    pub signers: Vec<Address, 10>, // max 10 elements, read through signers()
}
```

`String<MAX>` and `Vec<T, MAX>` declare bounded compact account fields without adding a lifetime to the account struct. Their generated accessors return `&str` and `&[T]` views into account data. The capacity is enforced when the account is initialized or mutated.

Use `log()` instead of `println!()` for on-chain logging.

## Quick reference

| Anchor | Quasar |
|--------|--------|
| `anchor_lang::prelude::*` | `quasar_lang::prelude::*` |
| `Context<T>` | `Ctx<T>` |
| `Result<()>` | `Result<(), ProgramError>` |
| `Signer<'info>` | `Signer` |
| `#[account(mut)]` | `#[account(mut)]` |
| `#[account]` | `#[account(discriminator = N)]` |
| Discriminator optional | `#[instruction(discriminator = N)]` required |
| `CpiContext::new(...)` | `.transfer(...).invoke()` |
| Field-level `seeds` / `bump` | Typed `#[seeds(...)]`, `address = Type::seeds(...)`, and `self.field_signer(bumps)` |
| `String` / `Vec` (heap) | `String<MAX>` / `Vec<T, MAX>` (zero-copy account fields) |
