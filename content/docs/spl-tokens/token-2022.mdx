...
```rust
use quasar_spl::{Mint2022, Token2022, Token2022Program, TokenCpi};

#[derive(Accounts)]
pub struct Transfer2022 {
...
|----------|-------------|-------------|
| SPL Token only | `Account<Token>` / `Account<Mint>` | `Program<TokenProgram>` |
| Token-2022 only | `Account<Token2022>` / `Account<Mint2022>` | `Program<Token2022Program>` |
...
The `token(...)` behavior works with interface types. Pass the token program field so initialization uses the program selected by the caller:
