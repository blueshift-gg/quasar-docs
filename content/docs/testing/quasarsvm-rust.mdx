...
QuasarSVM is a lightweight Solana VM harness built for Quasar. It supports deploying programs, sending transactions, inspecting account state, and checking compute unit usage -- all from Rust, without starting a validator.
...
After processing a transaction, the result contains the modified account state:
...
Chain multiple instructions by feeding resulting accounts from one transaction into the next:
...
The transaction result includes the compute units consumed, so you can assert budget limits or track CU regressions:
...
A complete test file for the [vault example program](/docs/getting-started/quick-start), testing both deposit and withdraw flows:
...
This builds the program, generates the client, and runs `cargo test`. Filter by test name:
