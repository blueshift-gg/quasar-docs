...
- An `EscrowInner` value and `set_inner()` method because the account opted into `set_inner`

The generated inner value keeps the native field types you declared and `set_inner()` converts them to Pod internally:
...
self.escrow.set_inner(EscrowInner {
    ...
    maker_ta_b: *self.maker_ta_b.address(), // Address
...
Variable-length fields use `String<MAX>` and `Vec<T, MAX>`. Quasar stores their length prefixes in the compact account header and their current payloads in the tail:
...
Read accessors borrow the validated account buffer:
...
To change a dynamic field, create a mutation guard with the account that pays or receives lamports during resize:
...
The guard saves on drop and resizes the account to the exact compact length. Borrowed `&str` and `&[T]` values are instruction argument types, not account field types.
...
`init` creates the account via a CPI to the System program -- it allocates space, pays rent, writes the discriminator, and assigns ownership to your program in one step. `init(idempotent)` skips creation when the account is already initialized, then validates the existing account.

Both need a payer. If your struct has a field named `payer`, it's auto-detected. Otherwise specify it explicitly with `payer = <field>`. Idempotent initialization also requires either an `address` constraint or an account behavior group, such as the token constraints below, that can validate an existing account. Space is auto-computed from the account type; override with `space = <expr>` for dynamic accounts or custom sizing:
...
// Idempotent initialization for token accounts
#[account(
    init(idempotent),
    payer = taker,
...
The `quasar-spl` crate provides account behaviors for SPL token accounts, mints, and associated token accounts. Each behavior validates existing account data and supplies the token-program-specific CPI setup during initialization:
...
#[account(
    init,
    payer = payer,
    mint(decimals = 6, authority = authority, token_program = token_program)
)]
pub mint: Account<Mint>,
...
See the [Account Constraints Reference](/docs/references/account-constraints) for the complete behavior arguments, including `freeze_authority` for `mint(...)`, plus the metadata and master-edition behaviors.
