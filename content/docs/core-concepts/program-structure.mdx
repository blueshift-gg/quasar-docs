...
**`String<MAX>`** and **`Vec<T, MAX>`** store bounded dynamic data in the account's compact tail. Generated accessors return `&str` and `&[T]` views without allocation or deserialization:
...
Account structs do not borrow their dynamic fields. Borrowed `&str` and `&[T]` values are available for bounded instruction arguments with `#[max(N)]`, not for account storage.
...
## Build on the generated structure
