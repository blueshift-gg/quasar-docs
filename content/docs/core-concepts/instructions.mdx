...
Variable-length arguments use `String<MAX>` and `Vec<T, MAX>`. `String` uses a `u8` length prefix by default, while `Vec` uses `u16`. Quasar validates the compact header and produces `&str` or `&[T]` slices pointing directly into instruction data:
...
Inside the handler, `label` is a `&str`. The macro reads its one-byte length prefix, validates the `MAX` bound and UTF-8, and produces a slice into the instruction data. No allocation.

Wire format:
```text
[0x02]                         <- discriminator (1 byte)
[len: 1 byte]                  <- u8 string length
...
`ctx.remaining_accounts()` returns the raw trailing-account accessor. Parse a homogeneous list into `Remaining<T, N>` at the instruction boundary so the handler receives at most `N` values that already satisfy `T`. Keep the raw accessor for heterogeneous tails that require explicit per-entry checks.
...
pub struct UpdateProfile {
    #[account(constraints(profile.authority == expected_authority))]
...
After the handler returns `Ok(())`, the framework runs the epilogue -- `close` operations execute for any accounts marked with `close(dest = destination)`. If the handler returns an error, the epilogue is skipped and the entire transaction rolls back.

## Connect instructions to the rest of the program
