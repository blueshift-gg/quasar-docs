...
Instruction discriminators can be zero. `quasar add --instruction` inserts an unpinned `#[instruction]`; the `#[program]` macro assigns the next free one-byte discriminator in source order and skips explicit values.
...
pub struct Profile {
    ...
    pub name: String<64, u16>,       // 2-byte prefix + 0..64 data
    pub tags: Vec<Address, 10, u8>,  // 1-byte prefix + 0..320 data
}
...
|------------|--------|------------|
| `String<N>` | `u8` (1 byte) | 0..N |
| `String<N, u16>` | `u16` (2 bytes) | 0..N |
| `String<N, u32>` | `u32` (4 bytes) | 0..N |
| `Vec<T, N>` | `u16` (2 bytes) | 0..N * sizeof(T) |
| `Vec<T, N, u8>` | `u8` (1 byte) | 0..N * sizeof(T) |
| `Vec<T, N, u32>` | `u32` (4 bytes) | 0..N * sizeof(T) |

`String` and `Vec` are bounded account-field types. `N` is required and the optional final argument selects the prefix type:

```rust
// String<MAX, P> where P defaults to u8
pub tag: String<32>,          // u8 prefix, max 32 bytes
pub bio: String<4096, u16>,  // u16 prefix, max 4096 bytes

// Vec<T, MAX, P> where P defaults to u16
pub members: Vec<Address, 8>,       // u16 prefix, max 8 elements
pub scores: Vec<PodU64, 4, u8>,    // u8 prefix, max 4 elements
```

The equivalent numeric prefix widths are `1`, `2`, `4`, and `8`. Borrowed `&str` and `&[T]` values are supported for bounded instruction arguments, not account fields.
...
| `String<N>` | `string` + size-prefixed tail codec (`u8`, max N bytes) | `string` | Generated UTF-8 codec |
| `Vec<T, N>` | `{ vec: T }` + size-prefixed tail codec (`u16`, max N items) | `Array<T>` | Generated item codec |
| `#[max(N)] &str` (instruction argument) | Same compact `string` shape | `string` | Generated UTF-8 codec |
| `#[max(N)] &[T]` (instruction argument) | Same compact `{ vec: T }` shape | `Array<T>` | Generated item codec |
