...
Used with `init` or `init(idempotent)` to initialize a token account. `token::mint` and `token::authority` must be specified together.
...
| **Requires** | `init` or `init(idempotent)`, paired with `token::authority` |
...
|---|---|
| **Syntax** | `#[account(init, payer = p, token::mint = mint_field, token::authority = auth_field)]` |
| **Requires** | `init` or `init(idempotent)`, paired with `token::mint` |
