...
description: Complete reference for all #[account(...)] constraint attributes in Quasar.
---

Account constraints are specified as attributes on fields within a `#[derive(Accounts)]` struct using the `#[account(...)]` syntax. They generate compile-time validation code that runs during account parsing.
...
The `has_one`, `constraint`, and `address` attributes support custom error codes using the `@` syntax:
...
Marks the account as mutable. Generates a runtime check that the account was passed as writable in the transaction.
...
Allows this account to share the same address as a previous account in the struct. By default, Quasar rejects duplicate accounts to prevent aliasing bugs. Use `dup` when intentional duplication is needed.
...
Validates that the account's address matches a constant or expression. Supports custom errors with `@`.
...
Validates that a field stored in the account data matches the address of another account in the struct. The account type must have a field with the same name as the referenced account.
...
Evaluates an arbitrary boolean expression. If the expression returns `false`, the instruction fails. Supports custom errors with `@`.
...
Creates a new account via CPI to the System program. Automatically:
1. Derives the account size from `T::SPACE` (or `space = <expr>`)
2. Calculates rent-exempt lamports
3. Calls `create_account` CPI
4. Writes the discriminator bytes
...
Like `init`, but skips creation if the account already exists (discriminator is set). If the account is uninitialized (all zeros), it creates and initializes it.
...
Specifies which signer account pays for account creation. Must reference another field in the accounts struct.
...
Specifies the seeds used to derive a PDA. When combined with `bump`, the framework verifies that the account address matches the derived PDA.
...
Closes the account after the instruction executes:
1. Zeros the discriminator bytes
2. Drains all lamports to the destination account
3. Reassigns ownership to the System program
4. Resizes account data to zero
...
Resizes the account data to a new size, adjusting lamports for rent exemption. If growing, the payer provides additional lamports. If shrinking, excess lamports are returned to the payer.
...
Specifies the account that pays for (or receives excess from) reallocation.
...
These attributes are used with `init` or `init_if_needed` to initialize a token account with specific mint and authority. Both `token::mint` and `token::authority` must be specified together.
...
These attributes are used with `init` to initialize a new mint account.
...
These attributes handle associated token account creation and validation.

### `associated_token::mint = <field>`

Specifies the mint for ATA address derivation and validation.
...
Specifies the authority (wallet) for ATA address derivation and validation.
...
Specifies the token program for ATA derivation (needed when using Token-2022).
...
These attributes are used with `init` to create Metaplex metadata and master edition accounts.
