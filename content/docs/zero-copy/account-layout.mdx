...
**3. Trait impls and the opted-in initializer** -- `Discriminator` (the `[1]` byte), `Space` (total account size), `Owner` (your program's `crate::ID`), plus `EscrowInner` and `set_inner()` because the account requested them.
...
Writing with `set_inner()` takes the generated inner value. Its fields use the native types from the account declaration and are converted to Pod internally:
